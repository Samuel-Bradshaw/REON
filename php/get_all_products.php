<?php
   ...
   // Declaring an empty array to store the data we retrieve from the database in.
    ...
    try {
       $results = $pdo->query('SELECT product.*, category.category_name FROM product JOIN category WHERE product.category_id = category.category_id ORDER BY product.category_id');
