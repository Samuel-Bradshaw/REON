<?php
    ...
    $tile_picture2_position =  filter_var($obj->tile_picture2_position , FILTER_SANITIZE_STRING, FILTER_FLAG_ENCODE_LOW);
    $main_page_picture = filter_var($obj->main_page_picture , FILTER_SANITIZE_STRING, FILTER_FLAG_ENCODE_LOW);


    try {
        $query = "INSERT INTO category (`category_name`, `category_description`, `picture_1_filepath`, `picture_2_filepath`, `tile_picture_position`, `tile_picture2_position`, `main_page_picture`) VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        $insertItem->bindParam(6, $tile_picture2_position, PDO::PARAM_STR);
        $insertItem->bindParam(7, $main_page_picture, PDO::PARAM_STR);
        $insertItem->execute();
